...
*/
    ...
    public static class UntrustworthyMailWorker implements MailService {
        private final MailService[] thirdPartyPersonsList;
        private final RealMailService conspirator = new RealMailService();
        public UntrustworthyMailWorker(MailService[] thirdPartyPersons){
            this.thirdPartyPersonsList = thirdPartyPersons;
        }
        @Override
        public Sendable processMail(Sendable mail) {
            for (MailService mailService : thirdPartyPersonsList) {
                mail = mailService.processMail(mail);
            }
            return conspirator.processMail(mail);
        }
    ...
    public static class Thief implements MailService  {
        ...
        public Sendable processMail(Sendable mail) {
            if(mail instanceof MailPackage) {
                if(((MailPackage) mail).content.price >= price){
                    stolenValue += ((MailPackage) mail).content.price;
                    String from = ((MailPackage) mail).from;
                    String to = ((MailPackage) mail).to;
                    String oldContent = ((MailPackage) mail).content.content;
                    Package emptyBox = new Package("stones instead of " + oldContent, 0);
                    mail = new MailPackage(from, to, emptyBox);
                }
            }
            return mail;
        }
